...
5. description: Clear description of what this test validates
...
"""
    ...
    def analyze_failures_and_generate_more(self, failed_tests: List[Dict], num_additional: int = 20) -> List[Dict]:
        ...
        prompt = f"""
Analyze these failed API tests and generate {num_additional} additional targeted test cases to find more issues:

Failed Tests:
{json.dumps(failed_tests, indent=2)}

Based on the failure patterns, generate new test cases that:
1. Explore similar edge cases
2. Test related boundary conditions
3. Try variations of what failed
4. Test adjacent scenarios

Return ONLY a valid JSON array with the same structure as before.
"""
...
def main():
    ...
    with st.sidebar:
        ...
        # Gemini API Key
        st.subheader("🤖 Gemini AI Setup")
        gemini_api_key = st.text_input("Text Below API Key", type="password", 
                                       help="Get your free API key from https://makersuite.google.com/app/apikey")
        
        if gemini_api_key:
            st.success("✅ Gemini API Key configured")
        else:
            st.warning("⚠️ Enter Gemini API Key to enable AI test generation")
    ...
    # Main Content Tabs
    tab1, tab2, tab3 = st.tabs(["🤖 AI Test Generation", "📊 Test Results", "📥 Download Reports"])
    ...
    with tab1:
        ...
        with col1:
            ...
            sample_data = st.text_area("Provide a sample JSON structure for your API", 
        ...
        if st.button("🚀 Generate AI Test Cases", type="primary", disabled=not gemini_api_key):
            if not gemini_api_key:
                ...
                try:
                    ...
                    if test_cases:
                        ...
                        st.error("❌ Failed to generate test cases. Check your API key and try again.")
    ...
    with tab2:
        ...
        if st.session_state.test_results is None:
            ...
            if summary['failed'] > 0 and gemini_api_key:
                st.markdown("---")
